...
export default class JcodePlugin extends Plugin {
  ...
  async loadSettings() {
    ...
    if (!Array.isArray(this.settings.knownSessions))
      this.settings.knownSessions = [];
    const resumeId = this.settings.resumeSessionId?.trim() ?? "";
    if (resumeId)
      this.settings.activeSessionLabel =
        this.sessionDisplayName(resumeId) ||
        normalizeSessionLabel(this.settings.activeSessionLabel || "");
    const activeLabel = normalizeSessionLabel(
      this.settings.activeSessionLabel || "",
    );
    if (
      resumeId &&
      activeLabel &&
      !this.settings.knownSessions.some((s) => s.id === resumeId)
    ) {
      this.settings.knownSessions = upsertSavedSession(
        this.settings.knownSessions,
        {
          id: resumeId,
          label: activeLabel,
  ...
  private warmPersistentClient() {
    ...
    this.transport.start(
      ...
      (e) => {
        if (e.type === "status")
          this.statusBarItem?.setText(`jcode: ${e.detail}`);
  ...
  getActiveClientName() {
    return this.settings.resumeSessionId
      ? this.getClientDisplayLabel(
  ...
  listResumeSessions() {
    const byId = new Map<
      string,
      { id: string; label: string; lastUsedAt: string }
    >();
    for (const session of this.settings.knownSessions) {
      if (session.id?.trim())
        byId.set(session.id.trim(), {
          ...session,
          label: session.label || this.sessionDisplayName(session.id),
        });
    }
          ...
          try {
            ...
            return {
              ...
              label: normalizeSessionLabel(
                raw.title ||
                  raw.short_name ||
                  this.sessionDisplayName(id) ||
                  id,
              ),
              lastUsedAt:
                raw.updated_at || raw.last_active_at || raw.created_at || "",
            };
          } catch {
            const id = name.replace(/\.json$/, "");
            return {
              id,
              label: this.sessionDisplayName(id) || id,
              lastUsedAt: "",
            };
          }
        });
  ...
  getClientDisplayLabel(sessionId: string, label?: string) {
    const name =
      this.sessionDisplayName(sessionId) ||
      normalizeSessionLabel(label || "") ||
      sessionId;
  ...
  async syncActiveSessionLabelFromResumeId() {
    ...
    if (saved)
      this.settings.activeSessionLabel = this.sessionDisplayName(id) || saved;
  }
  ...
  async activateSavedSession(sessionId: string) {
    ...
    this.settings.activeSessionLabel =
      this.sessionDisplayName(id) || saved?.label || id;
  ...
  async startNewSessionFromSettings() {
    this.settings.resumeSessionId = "";
    this.settings.activeSessionLabel = normalizeSessionLabel(
      this.settings.activeSessionLabel || "",
    );
    await this.saveSettings();
    this.rebuildTransport();
  ...
  private async recordActiveSession(sessionId: string, label: string) {
    ...
    const cleanLabel =
      this.sessionDisplayName(cleanId) || normalizeSessionLabel(label);
  ...
  rebuildAutoTagger() {
    ...
    this.autoTagger = new AutoTagger(
      ...
      {
        mode: this.settings.autoTagMode,
        provider: this.settings.provider || undefined,
      },
    );
  }
  ...
  private async submitAskJcode(editor: Editor, view: MarkdownView) {
    ...
    if (!trigger) {
      ...
      new Notice(
        'jcode: no "/askjcode ..." line at cursor. Type /askjcode then your question.',
      );
      return;
    }
    ...
    const activeLabel =
      normalizeSessionLabel(this.settings.activeSessionLabel || "") ||
      deriveInitialSessionLabel(
        this.findCurrentHeading(editor),
        file?.basename ?? null,
      );
    ...
    try {
      const inserted = await runAskJcode(
        ...
        },
        {
          ...
          resolveDisplayTitle: () =>
            this.resolveDisplayTitleFromPrompt(trigger.prompt, vaultRoot),
        },
      );
      const synced = inserted
        ? await this.syncActiveSessionFromPrompt(trigger.prompt, vaultRoot)
        : null;
      if (synced)
        this.renameAdjacentJcodeCallout(
          editor,
          trigger.line,
          this.getClientDisplayLabel(synced.id, synced.label),
        );
      if (inserted)
        new Notice("jcode: done. Inserted response below the trigger line.");
    } finally {
      this.currentRequestActive = false;
    }
  }

  private async reconcileResumeSessionForVault(
    vaultRoot: string,
  ): Promise<boolean> {
    ...
    if (
      latest.id === this.settings.resumeSessionId &&
  ...
  private async syncActiveSessionFromPrompt(prompt: string, vaultRoot: string) {
    ...
    const hit =
      this.findSessionForCurrentTransport() ||
      this.findLatestSession({ prompt }) ||
      this.findLatestSession({ cwd: vaultRoot, prompt });
    if (!hit) return null;
    if (hit.id === this.settings.resumeSessionId) return hit;
    await this.recordActiveSession(hit.id, hit.label);
    this.statusBarItem?.setText(
      `jcode: active client → ${this.getClientDisplayLabel(hit.id, hit.label)}`,
    );
    return hit;
  }

  private async resolveDisplayTitleFromPrompt(
    prompt: string,
    vaultRoot: string,
  ) {
    const hit =
    ...
    const previousLabel = previousId
      ? this.getClientDisplayLabel(previousId, this.settings.activeSessionLabel)
      : "saved section";
    ...
    return {
      ...
      notice:
        hit.id !== previousId
          ? `Jcode switched section: ${previousLabel} → ${nextLabel}`
          : undefined,
    };
  }
  ...
  private renameAdjacentJcodeCallout(
    editor: Editor,
    triggerLine: number,
    label: string,
  ) {
    for (
      let line = triggerLine + 1;
      line < Math.min(editor.lineCount(), triggerLine + 5);
      line++
    ) {
  ...
  private findLatestSession(opts: {
    cwd?: string;
    status?: string;
    prompt?: string;
  }) {
    const sessions = this.discoverJcodeSessionsWithMeta()
      .filter((session) => {
        if (
          opts.cwd &&
          session.cwd &&
          path.resolve(session.cwd) !== path.resolve(opts.cwd)
        )
          return false;
        if (opts.status && session.status !== opts.status) return false;
        if (
          opts.prompt &&
          !this.sessionFileContainsPrompt(session.file, opts.prompt)
        )
          return false;
        return true;
      })
      ...
      return fs
        ...
        .filter(
          (
            ...
            ReturnType<JcodePlugin["readSessionMetaFile"]>
          > => Boolean(session),
        );
    } catch {
      return [];
  ...
  private readSessionMetaFile(file: string) {
    try {
      ...
      return {
        ...
        file,
        label: normalizeSessionLabel(
          raw.title || raw.short_name || this.sessionDisplayName(id) || id,
        ),
        lastUsedAt:
          raw.updated_at || raw.last_active_at || raw.created_at || "",
        ...
        pids: [
          raw.last_pid,
          ...(raw.env_snapshots ?? []).map((snapshot) => snapshot.pid),
  ...
  private sessionFileContainsPrompt(file: string, prompt: string) {
    ...
    try {
      ...
      return (
        haystack.includes(needle) || haystack.includes(needle.slice(0, 120))
      );
    } catch {
      return false;
    }
  ...
  private async maybeAutoTagEmptyNote(
    editor: Editor,
    view: MarkdownView,
  ): Promise<boolean> {
    const file = view.file;
    ...
    new Notice(
      "jcode auto-tag: empty note detected, suggesting tags from title only.",
    );
    await this.suggestTagsFor(file, true);
    return true;
  }
  ...
  private resolveActiveMarkdownView(): ViewWithEditor | null {
    ...
    return {
      file: view.file
        ? { path: view.file.path, basename: view.file.basename }
        : null,
      editor: {
  ...
  private async createOrOpenTasksDashboard() {
    const dashboardPath =
      this.settings.tasksDashboardPath?.trim() || DEFAULT_TASKS_DASHBOARD_PATH;
  ...
  private async runSpacedRepOncePerDay() {
    ...
    const data = (await this.loadData()) as JcodeSettings & {
      lastSpacedRepRunDate?: string;
    };
    if (data?.lastSpacedRepRunDate === today) return;
    await this.runSpacedRep(false);
    this.settings = {
      ...this.settings,
      lastSpacedRepRunDate: today,
    } as JcodeSettings & { lastSpacedRepRunDate: string };
    await this.saveData(this.settings);
  }
