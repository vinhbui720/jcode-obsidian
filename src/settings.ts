...
export class JcodeSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .addToggle((t) =>
        t
          .setValue(this.plugin.settings.contextBroadcastEnabled)
          .onChange(async (v) => {
    ...
    new Setting(containerEl)
      .setName("Transport")
      .setDesc(
        "How /askjcode talks to jcode. 'persistent REPL' keeps one live jcode client alive while Obsidian is open. This is the recommended mode for slash commands, live context, and client memory.",
      )
      .addDropdown((dd) =>
        dd
          ...
          .onChange(async (v) => {
            this.plugin.settings.transport = v as
              | "stdio"
              | "repl"
              | "websocket";
            await this.plugin.saveSettings();
    ...
    new Setting(containerEl)
      ...
      .addButton((b) =>
        b
          .setButtonText("Start new client")
          .setTooltip(
            "Clear current resume session and start a fresh Obsidian jcode client",
          )
          .onClick(async () => {
    ...
    new Setting(containerEl)
      ...
      .addDropdown((dd) => {
        ...
        for (const session of savedSessions) {
          const suffix =
            session.id === this.plugin.settings.resumeSessionId
              ? "  • active"
              : "";
          dd.addOption(
            session.id,
            `${this.plugin.getClientDisplayLabel(session.id, session.label)}${suffix}`,
          );
        }
        dd.setValue(this.plugin.settings.resumeSessionId || "__new__").onChange(
          async (v) => {
            if (v === "__new__")
              await this.plugin.startNewSessionFromSettings();
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Token from `jcode pair`. Stored only in this vault's plugin data.",
      )
      .addText((t) =>
        t
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Show /askjcode token-by-token progress in the Obsidian status bar.",
      )
      .addToggle((t) =>
        t
          .setValue(this.plugin.settings.statusBarStreaming)
          .onChange(async (v) => {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "One glob per line. Matching files are skipped (e.g. templates/**).",
      )
      .addTextArea((t) =>
        t.setValue(this.plugin.settings.todoIgnoreGlobs).onChange(async (v) => {
    ...
    containerEl.createEl("h3", { text: "Obsidian Tasks plugin helper" });
    containerEl.createEl("p", {
      text: "Creates a dashboard note powered by the community Tasks plugin. Use command palette: jcode Tasks: create/open dashboard.",
    });
    new Setting(containerEl)
      .setName("Tasks dashboard path")
      .setDesc(
        "Vault-relative note created/opened by the jcode Tasks dashboard command.",
      )
      .addText((t) =>
        t
          .setPlaceholder(DEFAULT_TASKS_DASHBOARD_PATH)
          .setValue(this.plugin.settings.tasksDashboardPath)
          .onChange(async (v) => {
            this.plugin.settings.tasksDashboardPath =
              v.trim() || DEFAULT_TASKS_DASHBOARD_PATH;
            await this.plugin.saveSettings();
          }),
      );
    new Setting(containerEl)
      .setName("Tasks global filter")
      .setDesc(
        "Optional filter inserted at the top of every generated Tasks query, e.g. #task. Leave blank if Tasks plugin global filter is empty.",
      )
      .addText((t) =>
        t
          .setPlaceholder("#task")
    ...
    containerEl.createEl("h3", {
      text: "B2 — Auto-tag from title (low AI cost)",
    });
    containerEl.createEl("p", {
    ...
    new Setting(containerEl).setName("Enable auto-tag").addToggle((t) =>
    ...
    new Setting(containerEl)
      .setName("Mode")
      .setDesc(
        "'suggest' = preview only. 'auto' = apply tags into frontmatter immediately.",
      )
      .addDropdown((d) =>
        d
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Run automatically whenever a new .md file appears in the vault.",
      )
      .addToggle((t) =>
    ...
    containerEl.createEl("h3", {
      text: "B5 — Spaced repetition daily picker (no AI)",
    });
    containerEl.createEl("p", {
    ...
    new Setting(containerEl)
      ...
      .addToggle((t) =>
        t
          .setValue(this.plugin.settings.spacedRepEnabled)
          .onChange(async (v) => {
            this.plugin.settings.spacedRepEnabled = v;
            await this.plugin.saveSettings();
          }),
      );
    new Setting(containerEl).setName("Output note path").addText((t) =>
      t
        .setPlaceholder("today-review.md")
        .setValue(this.plugin.settings.spacedRepOutputPath)
        .onChange(async (v) => {
          this.plugin.settings.spacedRepOutputPath =
            v.trim() || "today-review.md";
          await this.plugin.saveSettings();
        }),
    );
    new Setting(containerEl).setName("Daily pick count").addText((t) =>
      t
        .setValue(String(this.plugin.settings.spacedRepDailyPickCount))
        .onChange(async (v) => {
          const n = Number.parseInt(v, 10);
          if (Number.isFinite(n) && n > 0) {
    ...
    new Setting(containerEl).setName("Default interval days").addText((t) =>
      t
        .setValue(String(this.plugin.settings.spacedRepDefaultInterval))
        .onChange(async (v) => {
          const n = Number.parseInt(v, 10);
          if (Number.isFinite(n) && n > 0) {
    ...
    new Setting(containerEl).setName("#spaced-rep tag boost").addText((t) =>
      t
        .setValue(String(this.plugin.settings.spacedRepTagBoost))
        .onChange(async (v) => {
    ...
    new Setting(containerEl)
      ...
      .addTextArea((t) =>
        t
          .setValue(this.plugin.settings.spacedRepIgnoreGlobs)
          .onChange(async (v) => {
